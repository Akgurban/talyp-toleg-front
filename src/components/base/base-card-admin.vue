  ...
  <div class="bg-img w-full relative rounded-2xl">
    ...
    >
      ...
      <div>
        <baseIcon
          name="delete"
          @click="$emit('delete', props.info.paymentID)"
    ...
    <div class="mx-auto flex items-center justify-center mt-2">
      <div class="px-12 w-full pb-[10px]">
        <div class="flex justify-between px-4 pt-2 pb-2">
          <div class="flex gap-2">
            <img src="/cash.svg" alt="" />
            <h1>
              {{
                props.info.paymentType == "1"
                  ? "I semester  "
                  : props.info.paymentType == "2"
                  ? " II semester"
                  : "I, II semester"
              }}
            </h1>
        ...
        <Image alt="Image" preview class="w-full">
          ...
          <template #image>
            <BaseImg
              ...
              class="w-full aspect-[26/7] object-cover"
              alt="image"
              width="250"
            />
          </template>
    ...
    <div class="flex justify-center gap-5 px-8 pt-10 pb-10">
      <Button
        @click="$emit('typeUpdate', 1, props.info.paymentID)"
        severity="success"
      >
        <BaseIcon name="doubleCheck"></BaseIcon> Tassykla
      </Button>
      <Button
        @click="$emit('typeUpdate', 2, props.info.paymentID)"
        severity="danger"
        ><BaseIcon name="cancel"></BaseIcon> Aýyr</Button
      >
    </div>
  </div>
</template>
